...
1. **Décompresser** l'archive dans un dossier local. L'emplacement recommandé
   est `%LOCALAPPDATA%\Programs\LinkCsvSage`.
   ...
   `%LOCALAPPDATA%` plutôt que `C:\Program Files (x86)` pour une seule raison :
   l'utilisateur peut y écrire, donc **les mises à jour s'installent sans
   demander d'administrateur**. Sous `Program Files`, chacune en réclamera un.
   Les deux emplacements fonctionnent ; le second suppose qu'un administrateur
   soit disponible à chaque version.

   Sur un poste partagé par plusieurs utilisateurs Windows, `Program Files`
   reste préférable : une seule installation à tenir, au prix d'une élévation
   par mise à jour.
...
L'application s'en charge. Au lancement, une fois par jour, elle interroge le
dépôt public des publications ; si une version plus récente existe, elle la
propose avec la liste de ce qui change.

Accepter suffit : le paquet est téléchargé, vérifié, l'application se ferme, se
remplace et se rouvre. Rien d'autre n'est à faire.

Trois choses à savoir :

- **elle ne se met jamais à jour toute seule.** La proposition attend une
  réponse. Refuser est sans conséquence, et *Ignorer cette version* ne porte que
  sur celle-là — la suivante sera proposée ;
- **rien n'est vérifié quand Sage la déclenche.** L'application est alors lancée
  sur un document ouvert, et une fenêtre surgissant par-dessus Sage au milieu
  d'une saisie serait déplacée ;
- **le paquet reçu est du même mode que celui installé.** Une installation
  dépendante du framework ne se verra jamais proposer le paquet autonome, ni
  l'inverse.

Les paramètres vivant dans `%APPDATA%`, ils survivent à la mise à jour. Le
dossier d'installation ne contient rien de personnel : il est écrasé
intégralement, après que son contenu a été mis de côté le temps de l'opération.

### Si une mise à jour échoue

**La version précédente est remise en place**, et l'application reste utilisable.
Le message le dit, et le journal en conserve le détail. Il n'y a rien à réparer :
relancer la mise à jour plus tard suffit.

Le seul cas où quelque chose reste à faire à la main est annoncé explicitement,
avec le chemin du dossier `.maj-precedente` où se trouvent les fichiers d'origine.

### Droits nécessaires

Installée dans `%LOCALAPPDATA%\Programs\LinkCsvSage`, l'application se met à jour
**sans rien demander**. Installée dans `C:\Program Files (x86)`, chaque mise à
jour demande une autorisation d'administrateur — Windows protège ce dossier.

C'est la raison de l'emplacement recommandé plus haut. Les deux fonctionnent ;
seul le second suppose qu'un administrateur soit disponible à chaque version.

### Recherche manuelle

*À propos* — le numéro de version dans la barre d'état — porte un bouton
**Rechercher une mise à jour**. C'est aussi le seul moyen de revenir sur un
*Ignorer cette version*.

### Mise à jour manuelle

Elle reste possible, et c'est la seule voie sur un poste sans accès à Internet :

1. Fermer l'application sur le poste.
2. Remplacer le contenu du dossier d'installation par celui du nouveau paquet.
3. Relancer.

### Couper la vérification

Un site dont la politique interdit les appels sortants peut la désactiver, en
passant `"CheckForUpdates": false` dans `%APPDATA%\LinkCsvSage\settings.json`.
Sans cela, la tentative échoue chaque jour sans conséquence, mais laisse une
trace au journal.
